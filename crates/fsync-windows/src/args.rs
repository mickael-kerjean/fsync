...
use clap::Parser;
use fsync_core::sdk::normalize_server;

use crate::gui::{self, Credentials};

#[derive(Parser)]
#[command(name = "fsync-windows", about = "Filestash drive client — Windows")]
struct Args {
...
pub fn init() -> Result<Option<Setup>, Box<dyn std::error::Error>> {
    ...
    match setup(args, fsync_core::config::recall(&data).map(Credentials::from), root, data) {
...
fn setup(
    ...
    let config_path = args.config.unwrap_or_else(|| data.join("fsync.toml"));
    Ok(Setup {
        root,
        data,
        config_path,
