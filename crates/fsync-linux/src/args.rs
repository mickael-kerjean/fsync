...
use clap::Parser;
use fsync_linux::gui::{self, Credentials};

#[derive(Parser)]
#[command(name = "fsync", about = "Filestash drive client")]
struct Args {
...
pub fn init() -> Result<Setup, Box<dyn std::error::Error>> {
    ...
    let setup = setup(args, fsync_core::config::recall(&data).map(Credentials::from))?;
...
mod tests {
    ...
    fn parse(argv: &[&str]) -> Args {
        Args::try_parse_from([&["fsync", "/tmp/mnt"], argv].concat()).unwrap()
    }
    ...
    fn conflicting_and_orphan_flags_are_rejected() {
        ...
        assert!(Args::try_parse_from(["fsync"]).is_err());
    }
}
