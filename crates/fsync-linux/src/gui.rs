...
impl From<fsync_core::config::Session> for Credentials {
    fn from(session: fsync_core::config::Session) -> Self {
        Self {
            url: session.url,
            token: session.token,
...
pub use fsync_core::sdk::normalize_server;
...
fn ensure_icons(dir: &Path) -> std::io::Result<()> {
    const ICONS: [(&str, &str); 4] = [
        (
            "icon-base.svg",
            include_str!("../../fsync-core/icons/icon-base.svg"),
        ),
        (
            "icon-ok.svg",
            include_str!("../../fsync-core/icons/icon-ok.svg"),
        ),
        (
            "icon-sync.svg",
            include_str!("../../fsync-core/icons/icon-sync.svg"),
        ),
        (
            "icon-error.svg",
            include_str!("../../fsync-core/icons/icon-error.svg"),
        ),
    ];
...
fn show_login(prefill: Credentials) -> Option<Credentials> {
    ...
    if let Err(err) = fsync_core::sdk::Sdk::builder(&url)
